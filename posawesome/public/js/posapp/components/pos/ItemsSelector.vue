...
export default {
  ...
  watch: {
    ...
    new_line() {
      eventBus.emit("set_new_line", this.new_line);
    },
  },

  methods: {
    show_offers() {
      eventBus.emit("show_offers", "true");
    },
    show_coupons() {
      eventBus.emit("show_coupons", "true");
    },
    get_items() {
      if (!this.pos_profile) {
      ...
      if (
        ...
        eventBus.emit("set_all_items", vm.items);
        vm.loading = false;
      }
      frappe.call({
  ...
  created: function () {
    ...
    eventBus.on("update_cur_items_details", () => {
      this.update_cur_items_details();
    });
    eventBus.on("update_offers_counters", (data) => {
      this.offersCount = data.offersCount;
      this.appliedOffersCount = data.appliedOffersCount;
    });
    eventBus.on("update_coupons_counters", (data) => {
      this.couponsCount = data.couponsCount;
      this.appliedCouponsCount = data.appliedCouponsCount;
    });
    eventBus.on("update_customer_price_list", (data) => {
      this.customer_price_list = data;
    });
    eventBus.on("update_customer", (data) => {
      this.customer = data;
    });
  },
