<template>
  ...
  <v-card class="mt-2 p-3">
    <v-row align="center" justify="end">
      <v-col cols="8" class="p-3">
        <v-row>
...
export default {
  ...
  methods: {
    ...
    new_invoice(data = {}) {
      ...
      if (!data.name && !data.is_return) {
        ...
        if (data.is_return) {
          eventBus.emit("set_customer_readonly", true);
    ...
    async show_payment() {
      ...
      if (this.invoice_doc.doctype == "Sales Order") {
        eventBus.emit("show_payment", "true");
        const invoice_doc = await this.process_invoice_from_order();
        eventBus.emit("send_invoice_doc_payment", invoice_doc);
    ...
    get_draft_invoices() {
      ...
      frappe.call({
        ...
        callback: function (r) {
          if (r.message) {
            eventBus.emit("open_drafts", r.message);
          }
        },
      });
    },
    ...
    get_draft_orders() {
      ...
      frappe.call({
        ...
        callback: function (r) {
          if (r.message) {
            eventBus.emit("open_orders", r.message);
          }
        },
      });
    },
    ...
    open_returns() {
      eventBus.emit("open_returns", this.pos_profile.company);
    },
    ...
    update_price_list() {
      ...
      eventBus.emit("update_customer_price_list", price_list);
    },
    update_discount_umount() {
    ...
    updatePosOffers(offers) {
      eventBus.emit("update_pos_offers", offers);
    },
  ...
  mounted() {
    ...
    eventBus.on("load_invoice", (data) => {
      ...
      if (this.invoice_doc.is_return) {
        ...
        eventBus.emit("set_pos_coupons", data.posa_coupons);
      }
    });
    eventBus.on("load_order", (data) => {
      this.new_order(data);
      // eventBus.emit("set_pos_coupons", data.posa_coupons);
    });
    eventBus.on("set_offers", (data) => {
      this.posOffers = data;
    });
    eventBus.on("update_invoice_offers", (data) => {
      this.updateInvoiceOffers(data);
    });
    eventBus.on("update_invoice_coupons", (data) => {
      this.posa_coupons = data;
      this.handelOffers();
    });
    eventBus.on("set_all_items", (data) => {
      this.allItems = data;
      this.items.forEach((item) => {
        this.update_item_detail(item);
      });
    });
    eventBus.on("load_return_invoice", (data) => {
  ...
  watch: {
    customer() {
      this.close_payments();
      eventBus.emit("set_customer", this.customer);
      this.fetch_customer_details();
      this.set_delivery_charges();
    },
    customer_info() {
      eventBus.emit("set_customer_info_to_edit", this.customer_info);
    },
    expanded(data_value) {
    ...
    discount_percentage_offer_name() {
      eventBus.emit("update_discount_percentage_offer_name", {
    ...
    invoiceType() {
      eventBus.emit("update_invoice_type", this.invoiceType);
    },
    discount_amount() {
